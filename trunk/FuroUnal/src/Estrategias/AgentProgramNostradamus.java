 ...
 */
public class AgentProgramNostradamus implements AgentProgram
{

	public final static double PI=3.1415923;
	public double ox=100;//solo funciona para equipo a la Izquierda
	public double oy=90;//solo funciona para equipo a la Izquierda
	public final static double KVp=5;
	public final static double KVd=4;
	public final static double Kp=20;
	public final static double Kd=40;
	public final static double v=300;
	...
	public Action compute(Percept p) 
	{
		...
		{	
			Atacabola((Vector<Robot>) p.getAttribute("home"),predicebolaX()+70,predicebolaY(),p);	
		}
		else
		{
			if(Math.abs(((Vector<Robot>) p.getAttribute("home")).elementAt(id).getPos().getX()-ox)<5&&Math.abs(((Vector<Robot>) p.getAttribute("home")).elementAt(id).getPos().getY()-oy)<5)
			{
				vleft=0;
				vright=0;
			}
			else
			Regresa((Vector<Robot>)p.getAttribute("home"),ox,oy,ox+5,oy);
		}
		Yant=by;
		Xant=bx;
	...
	public AgentProgramNostradamus(int _id)
	{
		id=_id;
		init();
	...
	{
		if(id<5)
		AreaAccion=new Bounds(0,220,180,0);
		else
		{
			AreaAccion=new Bounds(0,80,180,0);
			ox=40;
		}
	...
	{
		X=predicebolaX()-_home.elementAt(id).getPos().getX();
		Y=predicebolaY()-_home.elementAt(id).getPos().getY();
