...
public class AgentProgramPorrista implements AgentProgram
{

	public final static double PI=3.1415923;
	public final static double ox=15;//solo funciona para equipo a la derecha
	public final static double oy=90;//solo funciona para equipo a la derecha
	public final static double KVp=1;
	public final static double KVd=1;
	public final static double Kp=14;
	public final static double Kd=14;
	...
	public AgentProgramPorrista(int _id)
	{
		init();
		id=_id;
		j=0;
	...
	public Action compute(Percept p) 
	{
		...
		Vector<Robot> home=(Vector<Robot>)p.getAttribute("home");
		Ball ball=(Ball)p.getAttribute("ball");
		
		//System.out.print("bola:" + ball.getPos().getX() + " "+ ball.getPos().getY()+"\n");
		//System.out.print("vel:" + vleft + " "+ vright+"\n");
		bx=ball.getPos().getX();
		by=ball.getPos().getY();
		//System.out.print("Yball:" + by + " YRobot "+ home.elementAt(0).getPos().getY()+"\n");
		Y=by-home.elementAt(id).getPos().getY();
		X=bx-home.elementAt(id).getPos().getX();
		a=home.elementAt(id).getRotation();
		
		//System.out.println("Angulo="+a);
		//System.out.print("X:" + home.elementAt(0).getPos().getX() + "Y "+ home.elementAt(0).getPos().getY() +"Angulo: "+a+"\n");
		//System.out.print("cuadro:" + home.elementAt(0).getRotation() +"\n");
		//if(Math.abs(home.elementAt(0).getPos().getX()-ox)<=100){//esta cerca a su arco		
		
		
		//if(home.elementAt(id).getPos().getX()<110&&j==0)
		//{
			//Atacabola(home,bx+10,by,p);
		//}
		//else
		//{
		//j=1;	
		if(Math.abs(home.elementAt(id).getPos().getY())>20)//esta cerca a su arco
			Regresa(home,home.elementAt(id).getPos().getX()+10,4,home.elementAt(id).getPos().getX()+10,2);
		else
		{
			vleft=0;
			vright=0;
	...
	public void init() 
	{
		 AreaAccion=new Bounds(8,50,130,50);		 
		 eant=0;
		 Yant=0;
		 vleft=0;
		 vright=0;
	...
	private void BloqueaArquero(Vector<Robot> _home)//movimiento vertical
	{
		...
		{
			Y=AreaAccion.getBottom()-_home.elementAt(id).getPos().getY();
		}
		if(by>AreaAccion.getTop())
			Y=AreaAccion.getTop()-_home.elementAt(id).getPos().getY();
	...
	{
		X=x-_home.elementAt(id).getPos().getX();
		Y=y-_home.elementAt(id).getPos().getY();
		double phi=Math.atan2(Y,X);
		X=fx-_home.elementAt(id).getPos().getX();
		Y=fy-_home.elementAt(id).getPos().getY();
	...
	{
		X=bx-_home.elementAt(id).getPos().getX();
		Y=by-_home.elementAt(id).getPos().getY();
		double phi=Math.atan2(Y,X);
		X=_x-_home.elementAt(id).getPos().getX();
		Y=_y-_home.elementAt(id).getPos().getY();
		double alpha=Math.atan2(Y,X)-phi;
		phi=phi-alpha;
		double e=0;
		if( Math.abs(phi) <= PI/2)
